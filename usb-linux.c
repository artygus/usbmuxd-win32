...
static void usb_disconnect(struct usb_device *dev)
{
    ...
    FOREACH(struct libusb_transfer *xfer, &dev->tx_xfers, struct libusb_transfer *) {
...
{
    struct usb_device *dev = (usb_device *)xfer->user_data;
...
int usb_send(struct usb_device *dev, const unsigned char *buf, int length)
{
    ...
    libusb_fill_bulk_transfer(xfer, dev->dev, dev->ep_out, (unsigned char *)buf, length, tx_callback, dev, 0);
    ...
    if (length % dev->wMaxPacketSize == 0) {
        ...
        libusb_fill_bulk_transfer(xfer, dev->dev, dev->ep_out, (unsigned char *)buffer, 0, tx_callback, dev, 0);
        if((res = libusb_submit_transfer(xfer)) < 0) {
...
static int start_rx(struct usb_device *dev)
{
    ...
    buf = malloc(USB_MRU);
    libusb_fill_bulk_transfer(dev->rx_xfer, dev->dev, dev->ep_in, (unsigned char *)buf, USB_MRU, rx_callback, dev, 0);
...
int usb_discover(void)
{
    ...
    FOREACH(struct usb_device *usbdev, &device_list, struct usb_device *) {
        usbdev->alive = 0;
    } ENDFOREACH
    ...
    for(i=0; i<cnt; i++) {
        ...
        usbdev = (usb_device *)malloc(sizeof(struct usb_device));
        memset(usbdev, 0, sizeof(*usbdev));

        for(j=0; j<config->bNumInterfaces; j++) {
            const struct libusb_interface_descriptor *intf = &config->interface[j].altsetting[0];

            /* me if(intf->bInterfaceClass != INTERFACE_CLASS ||
